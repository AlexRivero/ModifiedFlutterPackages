...
class Geodesy {
  ...
  maps.LatLng destinationPointByDistanceAndBearing(
      maps.LatLng l, num distance, num bearing,
      [num radius]) {
    ...
    return maps.LatLng(radianToDeg(latRadians2 as double),
        (radianToDeg(lngRadians2 as double) + 540) % 360 - 180);
  }

  /// calcuate the midpoint bewteen teo geo points
  maps.LatLng midPointBetweenTwoGeoPoints(maps.LatLng l1, maps.LatLng l2) {
    ...
    return maps.LatLng(radianToDeg(latRadians as double),
        (radianToDeg(lngRadians as double) + 540) % 360 - 180);
  }

  /// calculate the geo point of intersection of two given paths
  maps.LatLng intersectionByPaths(maps.LatLng l1, maps.LatLng l2, num b1, num b2) {
    num l1LatRadians = degToRadian(l1.latitude);
    num l1LngRadians = degToRadian(l1.longitude);
    num l2LatRadians = degToRadian(l2.latitude);
    ...
    return maps.LatLng(radianToDeg(lat3 as double),
        (radianToDeg(l3LngRadians as double) + 540) % 360 - 180);
  }

  /// calculate the distance in meters between two geo points
  num distanceBetweenTwoGeoPoints(maps.LatLng l1, maps.LatLng l2, [num radius]) {
    radius = radius ?? _RADIUS;
    num R = radius;
    num l1LatRadians = degToRadian(l1.latitude);
    num l1LngRadians = degToRadian(l1.longitude);
  ...
  num bearingBetweenTwoGeoPoints(maps.LatLng l1, maps.LatLng l2) {
    num l1LatRadians = degToRadian(l1.latitude);
    num l2LatRadians = degToRadian(l2.latitude);
    num lngRadiansDiff = degToRadian(l2.longitude - l1.longitude);
  ...
  num finalBearingBetweenTwoGeoPoints(maps.LatLng l1, maps.LatLng l2) {
  ...
  num crossTrackDistanceTo(maps.LatLng l1, maps.LatLng start, maps.LatLng end, [num radius]) {
  ...
  bool isGeoPointInBoudingBox(maps.LatLng l, maps.LatLng topLeft, maps.LatLng bottomRight) {
  ...
  bool isGeoPointInPolygon(maps.LatLng l, List<maps.LatLng> polygon) {
  ...
  /// Get a list of [maps.LatLng] points within a distance from
  /// a given point
  ///
  /// Distance is in meters
  List<maps.LatLng> pointsInRange(
      maps.LatLng point, List<maps.LatLng> pointsToCheck, num distance) {
    final geoFencedPoints = <maps.LatLng>[];
